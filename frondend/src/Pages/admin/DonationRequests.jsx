...
const DonationRequests = () => {
  ...
  const [filter, setFilter] = useState({ name: '', bloodGroup: '', status: '' });

  useEffect(() => {
    const fetchDonations = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/donate/donations');
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      const endpoint = status === 'Approved'
        ? `http://localhost:5000/api/donate/donations/approve/${id}`
        : `http://localhost:5000/api/donate/donations/reject/${id}`;
  ...
  const filteredByStatus = (status) =>
    filteredDonations.filter(donation => donation.status.toLowerCase() === status);
        ...
        <div className="scroll-container">
          {filteredByStatus('pending').length === 0 ? (
            ...
            filteredByStatus('pending').map((donation) => (
              <div key={donation._id} className={`request-card ${getStatusClass(donation.status)}`}>
                <h3>{donation.name}</h3>
